...
MStatus eae6320::cMayaCollisionDataExporter::writer(const MFileObject& i_file, const MString& i_options, FileAccessMode i_mode)
{
	MStatus status;
		...
		if (fout.is_open())
		{
				...
				"{\n";
			{
				...
				for (size_t i = 0; i < i_indexBuffer.size(); i += 3) {
					...
					{
						fout << "A={";
						fout << i_vertexBuffer[i_indexBuffer[i]].vertex.x << "," << i_vertexBuffer[i_indexBuffer[i]].vertex.y << "," << i_vertexBuffer[i_indexBuffer[i]].vertex.z;
						fout << "},\n";
					}
					{
						fout << "B={";
						fout << i_vertexBuffer[i_indexBuffer[i + 1]].vertex.x << "," << i_vertexBuffer[i_indexBuffer[i + 1]].vertex.y << "," << i_vertexBuffer[i_indexBuffer[i + 1]].vertex.z;
						fout << "},\n";
					}
					{
						fout << "C={";
						fout << i_vertexBuffer[i_indexBuffer[i + 2]].vertex.x << "," << i_vertexBuffer[i_indexBuffer[i + 2]].vertex.y << "," << i_vertexBuffer[i_indexBuffer[i + 2]].vertex.z;
						fout << "},\n";
					}
					fout << "},\n";
